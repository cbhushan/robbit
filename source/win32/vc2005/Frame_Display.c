...
void Frame_Display::draw()  {   
  ...
  glutSolidSphere(radius_ball,bot_slices, bot_stacks);
  glPopMatrix();
...
{
  char *temp_string;	// Temporary String
  ...
  if (!obstacle_data_file.is_open()) {
	...
	if(strncmp(string,"2D",2)==0)	{
	  // printf("|%s|\n", string);
	  temp_string = strtok (string," ("); 
	  temp_string = strtok (NULL, " ,"); obs_2D[obs_2D_counter][0] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " )"); obs_2D[obs_2D_counter][1] = strtod(temp_string, NULL); 
	  /*
	  obs_2D[obs_2D_counter][0]=strtod(string, &str_end);
	  obs_2D[obs_2D_counter][1]=strtod(str_end, &str_end);
	  */	  
	  obs_2D_counter++;
	}
			
	// Updates the Wall Obstacle
	else if(strncmp(string,"WL",2)==0) {
	  
	  temp_string = strtok (string," ("); 
	  temp_string = strtok (NULL, " ,"); obs_WL[WL_counter][1] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " ,"); obs_WL[WL_counter][2] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " ,"); obs_WL[WL_counter][3] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " ,"); obs_WL[WL_counter][4] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " )"); obs_WL[WL_counter][0] = strtod(temp_string, NULL);
	  /*		
	...
	else if(strncmp(string,"CB",2)==0) {
  	  temp_string = strtok (string," ("); 
	  temp_string = strtok (NULL, " ,"); obs_CB[CB_counter][1] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " ,"); obs_CB[CB_counter][2] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " )"); obs_CB[CB_counter][0] = strtod(temp_string, NULL);
	  /*
	...
	// Updates the Sphere Obstacle
   	  ...
   	  temp_string = strtok (string," ("); 
	  temp_string = strtok (NULL, " ,"); obs_SP[SP_counter][1] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " ,"); obs_SP[SP_counter][2] = strtod(temp_string, NULL);
	  temp_string = strtok (NULL, " )"); obs_SP[SP_counter][0] = strtod(temp_string, NULL);
	  /*
  ...
  // cout << obs_2D_counter << " " << WL_counter << " " << CB_counter << " " << SP_counter << endl;
...
void Frame_Display::DrawFloor()
{
  ...
  for(i=min_x+10; i<max_x+50; i+=80)
...
void Frame_Display::PositionCustomRobot(float pos_x,float pos_y,double orient,int colour) {
  ...
  glPushMatrix();
  glTranslatef(pos_x,pos_y,0);
  gluCylinder(quad,(this->radius_of_robot),(this->radius_of_robot),this->height_robot,this->bot_slices,this->bot_stacks);
  glPopMatrix();
